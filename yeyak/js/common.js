var  settingId = 'yeyak_setting'
	//,server = 'http://yeyak.gaeyou.com/api/v1/'
	//,category = 'game'
...
function getReplyList(reload) {
	...
	_oc.sendPost('yeyak_comment_list', data, 'cbPostYeyakCommentList');
}
	...
	if (result['result'] == 'success') {
		...
		if (rdata.length == 0) {
			...
			for (var i=rdata.length-1; i>=0; i--) {
				...
				str += '		<p class="profile"><img src="' + rdata[i]['story_profile'] + '"></p>';
				str += '	<div class="reply">';
		...
		$('[data-delReply]').on('click', function(){
			...
			if (confirm('댓글을 삭제하겠습니까?')) {
				...
				_oc.sendPost('yeyak_comment_delete', data, 'cbPostYeyakCommentDelete')
			}
			return false;
		})
...
function initReplyAdd() {
	...
	$('#btnAddComment').on('click', function(){
		...
		_oc.sendPost('yeyak_comment_insert', data, 'cbPostYeyakCommentInsert');
	})
}
