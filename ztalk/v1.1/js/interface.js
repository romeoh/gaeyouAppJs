var  server = 'http://ztalk.gaeyou.com/api/v1/'
	,platform = 'web'
	,urlParameter = {}
	,webAlarmTimeout = {}
	,_oc = {
	...
	// 링크
	'link': function(url, param, stack, orientation, animation) {
		var  param = param || ''
			,stack = stack || 'DEFAULT'
		...
		if (platform == 'android') {
			...
			window.openInterface.link(activity, param, stack, orientation, animation);
		} else {
			if (param != '') {
				param = '?' + param;
			}
	...
	'getDevice': function(callback) {
		if (platform == 'android') {
			window.openInterface.getDevice(callback);
		} else {
			var info = '{';
				info += '"phoneNumber": "", ';
				info += '"deviceId": "", ';
		...
		'story': {
			'post': {
				'note': function(text, callback) {
					if (platform == 'android') {
						window.openInterface.kakaoStoryPostNote(text, callback);
					} else {
					
					}
				},
				'image': function(imageUrl, text, callback) {
					if (platform == 'android') {
						window.openInterface.kakaoStoryPostImage(imageUrl, text, callback);
					} else {
					
					}
				},
				'link': function(url, text, callback) {
					if (platform == 'android') {
						window.openInterface.kakaoStoryPostLink(url, text, callback);
					} else {
