var  settingId = 'moa_game_setting'
	,category = 'game'
...
function request(tr, data, callback) {
	$.ajax({
		url : 'http://moa.gaeyou.com/api/v1/' + tr + '.php',
		dataType : "jsonp",
		data: data,
		jsonp: 'callback',
...
function getRss(url, callback) {
	$.ajax({
		//type: 'GET',
		url: 'http://moa.gaeyou.com/api/v1/rss.php',
		dataType : "jsonp",
		data: {'url': url},
		jsonp: 'callback',
		success:function(data) {
...
function removeTagAll(input, allowed) {
	...
	var  tags = /<\/?([a-z][a-z0-9]*)\b[^>]*>/gi
		...
		,resultTxt = input.replace(commentsAndPhpTags, '').replace(tags, '').replace(nbspTags, '')
	return {
